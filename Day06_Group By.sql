...
/*
GROUP BY : cümleciği bir SELECT ifadesinde satırları, sutunları değerlerine göreGENERATED
özet olarak gruplamak için kullanılır.
GROUP BY cümleciği her grup başına satır döndürür.
GROUP BY genelde, AVG(), COUNT(), MAX(). MİN() ve SUM() gibi aggregate 
...
select urun_adi, min(urun_miktari) as minumum_urun_mik, max(urun_miktari)as maksimun_urun_mik from manav 
where urun_adi is not null
group by urun_adi
order by maksimun_urun_mik desc;
...
select ulke, round (avg(maas),3) as ortalama_maas from personel
GROUP by ulke;


-- Maas ortalamasını bayanlar ve baylar olarak sorgulayınız.
select cinsiyet, avg(maas) as ortalama_maas from personel
group by cinsiyet;
...
select ulke, cinsiyet, count(*) as calisan_sayi, avg(yas) as ortalam_yas from personel
...
select ulke cinsiyet, count(*) as calisan_sayi, avg(yas) as ortalama_yas from personel
where maas>30000
group by ulke, cinsiyet;
